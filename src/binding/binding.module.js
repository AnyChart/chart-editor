 ...
 * @return {*}
 */
chartEditor.binding.init = function(opt_value) {
  ...
  if (goog.dom.isElement(opt_value)) {
    ...
    chartEditor.binding.setRealValue_(element);

    goog.events.listen(element, event, chartEditor.binding.onElementChange_, false);
 ...
 * @private
 */
chartEditor.binding.onElementChange_ = function(event) {
  ...
  if (chartId && chart && key) {
    ...
    chartEditor.binding.exec(chart, key, value);
  }
};
 ...
 * @private
 */
chartEditor.binding.setRealValue_ = function(element) {
  var type = element.type;
  if (!goog.isDef(type)) return;
  type = type.toLowerCase();
  ...
  if (chartId && chart && key) {
    var value = chartEditor.binding.exec(chart, key);
...
//exports
(function() {
  goog.exportSymbol('anychart.ui.binding.exec', chartEditor.binding.exec);
  goog.exportSymbol('anychart.ui.binding.init', chartEditor.binding.init);
})();
